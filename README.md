...
The example Bash script below cycles the execution of `phidget-rec` for `N` recording phases (first argument), each separated by `WAIT` days. These two arguments have to be given first and in that order, and followed by at least the compulsory arguments for `phidget-rec` (in any order) to suppress all input prompts and fully automate the whole routine. More complex scheduling could be achieved using `crontab` for instance.
